...
class Images extends CI_Controller {
	...
	public function ajax_change_image() {
		...
		if (IS_AJAX) {
			...
			switch($media) {
				case 'movies' :
					$json = array('success' => '1', 'url' => $url, 'message' => $this -> lang -> line('msg_' . $type . '_movie_updated'));
					break;

				case 'sets' :
					$json = array('success' => '1', 'url' => $url, 'message' => $this -> lang -> line('msg_' . $type . '_set_updated'));
					break;

				case 'tvshows' :
					$json = array('success' => '1', 'url' => $url, 'message' => $this -> lang -> line('msg_' . $type . '_tvshow_updated'));
					break;
			}
